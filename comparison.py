...
Original file is located at
    https://colab.research.google.com/drive/1R6zPJ0FzuWD2QDilXmPLbA6AOjgfaPgV
"""
...
print("  Werner theory: F>0.5 and C>0 both cross at p>1/3; CHSH needs p>1/sqrt(2)")
print("  Expected order: M1 (F) ≈ M4 (C) < M3 (CHSH)")
print("="*70)
...
SRC = {
    'F':     0.9759,
    'F_std': 0.0003,
    'C':     0.9482,                                    # paper Fig. 3a
    'C_std': 0.0007,
    'S':     2 * np.sqrt(2) * (4 * 0.9759 - 1) / 3,     # Werner = 2.7382
    'S_std': 2 * np.sqrt(2) * 4 * 0.0003 / 3,
}
...
configs = [
    ('S', 'S_std', 'nonlocal_M3', 2.0, TSIRELSON, SRC['S'],
     'M3: CHSH S_max (Horodecki 1995)', 'S_max', 'Classical S=2',
     f"Paper source S={SRC['S']:.4f}"),
    ('C', 'C_std', 'ent_M4', 0.0, 1.0, SRC['C'],
     'M4: Concurrence C (Wootters 1998)', 'Concurrence C', 'C=0 separable',
     f"Paper source C={SRC['C']:.4f}"),
    ('F', 'F_std', 'ent_M1', 0.5, 1.0, SRC['F'],
     'M1: Bell Fidelity F(|Psi-⟩)', 'Bell Fidelity F', 'Threshold F=0.5',
     f"Paper source F={SRC['F']:.4f}"),
]
